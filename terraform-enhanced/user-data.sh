...
if [ -d /home/ubuntu/deepseek-ocr.rs ]; then
    ...
    su - ubuntu -c 'cd /home/ubuntu/deepseek-ocr.rs && git fetch origin && git checkout feature/multimodal-ocr-with-bbox && git pull origin feature/multimodal-ocr-with-bbox'
else
    su - ubuntu -c 'git clone https://github.com/TimmyOVO/deepseek-ocr.rs.git /home/ubuntu/deepseek-ocr.rs'
    su - ubuntu -c 'cd /home/ubuntu/deepseek-ocr.rs && git checkout feature/multimodal-ocr-with-bbox'
fi
...
[Unit]
Description=DeepSeek OCR Server (Enhanced with Multimodal Support)
